...
class MusicService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            ...
            ACTION_STOP -> {
                stopPlayback()  // Stop playback when stop action is triggered
            }
        }
    ...
    private fun stopPlayback() {
        mediaPlayer.stop()
        mediaPlayer.reset()  // Reset the player so it can be reused
    ...
    private fun updateNotification(song: Song, isPlaying: Boolean) {
        ...
        notificationLayout.setOnClickPendingIntent(R.id.notification_stop, stopIntent)  // Stop button
    ...
    companion object {
        ...
        const val ACTION_STOP = "com.ex.musicplayer.STOP" 
